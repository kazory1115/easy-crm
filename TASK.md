...
- [x] 檢查並補齊所有已上線模組 permission
  ...
  - `report.*`
  - `staff.*`
- [x] 確認 `admin`、`manager`、`staff` 預設角色權限合理
- [x] 避免前端已使用但 seed 不存在的 permission name

### 2. 後端 API 權限防線落地

- [x] `/api/users` 系列 route 加上對應 `permission` middleware
- [x] 權限管理 route 僅允許 `role.manage` 或等效高權限角色使用
- [x] 補上未授權 / 無權限的 API response 測試

### 3. UserController / Service 補角色與權限管理

- [x] 建立 `UserPermissionService` 收斂角色與權限更新流程
- [x] `store/update/show` 回傳使用者時包含 roles / permissions
- [x] 新增更新角色 API：`syncRoles`
- [x] 新增更新直接權限 API：`syncPermissions`
- [x] 更新自己時要定義限制，避免 admin 把自己權限改到鎖死

### 4. Staff UI 補權限管理畫面

- [x] Staff 編輯 Modal 補角色設定區
- [x] Staff 編輯 Modal 補模組權限勾選區
- [x] 畫面明確區分：
  - 角色繼承權限
  - 直接指派權限
- [x] 權限欄位資料來源走 API，不要硬編碼散落在頁面

### 5. auth / route guard 對齊

- [x] `auth/me` 回傳資料格式與新的角色 / 權限治理一致
- [x] route guard 繼續用 `permissions` 判斷，但以後端回傳為準
- [x] 側邊欄模組顯示邏輯與 API seed 同步
...
- [x] admin 可更新某員工角色
- [x] admin 可更新某員工直接權限
- [x] 非 admin 無法更新他人角色 / 權限
- [x] 權限變更後重新登入或 `auth/me` 同步後，前端模組可見性正確

---

## 驗收標準

- [x] admin 可以在 Staff 模組中管理每位員工的角色與模組權限
- [x] 一般 staff 即使手動打 API，也不能修改他人權限
- [x] 員工被移除某模組權限後，前端入口、route、API 都一致被擋下
- [x] 員工被加上某模組權限後，重新同步登入狀態即可使用該模組
- [x] `order.*` 等既有漏 seed 的權限補齊，不再出現前後端 permission name 不同步
...
- [x] 已完成後端 `permission` middleware alias、模組 API 權限保護、`UserPermissionService`、`/api/users/{id}/roles`、`/api/users/{id}/permissions`、`/api/permissions/modules`
- [x] 已完成 Staff UI 角色與直接權限編輯畫面，並以 API 載入角色 / 模組權限 mapping
- [x] 已完成 sidebar 模組顯示改為依登入者權限過濾
- [ ] 自動測試完整驗證受環境限制
  - backend `php artisan test` 仍受 `pdo_sqlite` driver 缺失阻擋
  - frontend `npm.cmd run build` 仍受既有 `spawn EPERM` 環境問題阻擋
