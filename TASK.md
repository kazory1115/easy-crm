...
- [ ] 建立 dashboard 聚合 API
  ...
  - order stats
  - inventory stats
- [ ] 建立 export records API
- [ ] 建立 `reportApi.js`
- [ ] 建立 `ReportDashboard.vue`
- [ ] 建立 `ReportExportList.vue`
- [ ] 補 `report/routes.js`
- [ ] 補 `moduleConfig.report`
...
- [ ] 實作 Quote PDF 匯出
- [ ] 實作 Quote Excel 匯出
- [ ] 實作 `QuoteController@send`
- [ ] 建立 mail template
- [ ] 前端改為呼叫正式 export/send API
- [ ] 補失敗提示與下載處理
- [ ] 補測試
...
- [ ] 補 Auth / Quote / Staff / Order / CRM / Inventory 核心 feature tests
- [ ] 前端加入 Vitest 或 E2E 工具
- [ ] 補至少 2-3 條核心 smoke flow
- [ ] `package.json` 增加 test script
- [ ] 若可行，補 lint / format script
- [ ] 整理最小 CI 檢查清單

## 驗收標準

- [ ] backend tests 有可觀覆蓋，不再只靠 ExampleTest
- [ ] frontend 有最小測試基線
- [ ] 可用單一命令執行主要檢查
