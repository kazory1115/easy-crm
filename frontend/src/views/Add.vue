  ...
  <div id="app" class="max-w-7xl mx-auto mt-10">
    ...
    <div v-if="activeTab === 'items'" class="transition-opacity duration-300">
      <form
        ...
        class="bg-white rounded-2xl shadow-2xl border border-gray-100 p-8 mb-8"
      >
        ...
        <div class="grid md:grid-cols-4 gap-6 mb-6">
          ...
          <div>
            <label for="item-quantity" class="block mb-2 text-gray-700 font-medium"
              >數量</label
            >
            <input
              id="item-quantity"
          ...
          <table class="w-full min-w-[900px] text-sm">
            <thead>
              ...
              >
                ...
                <th class="py-3 text-center font-bold text-blue-800">品名規格</th>
                <th class="py-3 text-center font-bold text-blue-800 w-32">數量</th>
                <th class="py-3 text-center font-bold text-blue-800 w-24">單位</th>
                <th class="py-3 text-center font-bold text-blue-800 w-32">單價</th>
                <th class="py-3 text-center font-bold text-blue-800 w-32">複價</th>
                <th class="py-3 text-center font-bold text-blue-800 w-32">功能</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td v-if="editItemId !== item.id" class="text-center text-gray-800">
                  {{ item.name }}
                </td>
    ...
    <!-- 自定義模板 -->
    <div v-if="activeTab === 'templates'" class="transition-opacity duration-300">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <!-- 左側模板列表 -->
        <div class="md:col-span-1 bg-white rounded-2xl shadow-2xl border border-gray-100 p-6">
          <h3 class="text-2xl font-bold mb-4 text-blue-700">模板列表</h3>
          <button @click="createNewTemplate" class="w-full mb-4 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg shadow transition font-semibold flex items-center justify-center">
            <i class="fa-solid fa-plus mr-2"></i>新增模板
          </button>
          <ul class="space-y-2">
            <li v-for="template in templates" :key="template.id">
              <a @click.prevent="selectTemplate(template)" href="#" 
                 :class="[
                   'block w-full text-left px-4 py-3 rounded-lg transition-all duration-200',
                   activeTemplate && activeTemplate.id === template.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 hover:bg-blue-100 hover:text-blue-700'
                 ]">
                {{ template.name || '未命名模板' }}
              </a>
            </li>
             <li v-if="templates.length === 0" class="text-center text-gray-400 py-4">
                尚無模板資料
              </li>
          </ul>
        </div>
        ...
        <!-- 右側模板編輯器 -->
        <div class="md:col-span-2 bg-white rounded-2xl shadow-2xl border border-gray-100 p-8">
          ...
          <div v-else>
            <h3 class="text-2xl font-bold mb-6 text-gray-800">{{ isCreatingNew ? '新增模板' : '編輯模板' }}</h3>
            <form @submit.prevent="saveTemplate">
              <!-- 模板名稱 -->
              <div class="mb-6">
                <label class="block mb-2 text-gray-700 font-medium">模板名稱</label>
                <input v-model="activeTemplate.name" type="text" class="w-full px-4 py-3 rounded-lg border border-gray-300 text-gray-800 focus:border-blue-400 transition-all duration-200" placeholder="請輸入模板名稱" required />
              </div>

              <!-- 動態欄位 -->
              <h4 class="text-xl font-bold mb-4 text-gray-700">自定義欄位</h4>
              <div class="space-y-4 mb-6">
                <div v-for="(field, index) in activeTemplate.fields" :key="field.id" class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg border">
                  <div class="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label class="block mb-1 text-sm text-gray-600">欄位名稱</label>
                      <input v-model="field.label" type="text" class="w-full px-3 py-2 rounded-md border border-gray-300" placeholder="例如：CPU" required />
                    </div>
                    <div>
                      <label class="block mb-1 text-sm text-gray-600">預設值</label>
                      <input v-model="field.value" type="text" class="w-full px-3 py-2 rounded-md border border-gray-300" placeholder="例如：Intel i7" required />
                    </div>
                  </div>
                  <button @click.prevent="removeField(field.id)" class="flex-shrink-0 px-3 py-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition" title="刪除欄位">
                    <i class="fa-solid fa-trash-can"></i>
                  </button>
                </div>
                <div v-if="activeTemplate.fields.length === 0" class="text-center text-gray-400 py-4 border-2 border-dashed rounded-lg">
                  尚無自定義欄位
                </div>
              </div>
              <button @click.prevent="addField" class="mb-8 w-full bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg shadow-sm transition font-semibold flex items-center justify-center">
                <i class="fa-solid fa-plus mr-2"></i>新增欄位
              </button>

              <!-- 操作按鈕 -->
              <div class="flex justify-between items-center gap-4">
                <button v-if="!isCreatingNew" @click.prevent="deleteActiveTemplate" type="button" class="text-red-500 hover:text-red-700 font-semibold transition">
                  <i class="fa-solid fa-trash-can mr-2"></i>刪除模板
                </button>
                <div class="flex-grow"></div>
                <div class="flex gap-4">
                   <button @click.prevent="cancelEdit" type="button" class="px-8 py-3 bg-gray-400 text-white rounded-lg hover:bg-gray-500 transition font-semibold">
                    取消
                  </button>
...
function createNewTemplate() {
  ...
  activeTemplate.value = {
    name: '',
    fields: [],
    // 新增時也給予一個唯一的id，方便新增field時key的管理
